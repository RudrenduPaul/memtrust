...
- **Honest limitation -- read this before trusting a junk-retention number.** Every fake adapter
  ...
  real vendor's actual LLM-driven extraction pipeline -- these unit tests never talk to a live
  one). **This eval and its fixture prove the classification logic is correct against those fakes,
  and it has now also been run against a real, live backend: `mem0_direct` (self-hosted `mem0ai`,
  in-process, backed by local Qdrant + OpenAI) -- 12 junk cases all correctly rejected
  (`junk_retained_rate` 0%), but all 3 valid-content cases lost on retrieval (`valid_lost_rate`
  100%), the exact "discards everything, including the content it should have kept" failure shape
  this document already warns a bare 0% `junk_retained_rate` can hide (see the classification table
  above). It has still never been run at anything close to jamebobob's real 10,000+ entry scale.**
  A `junk_retained_rate` this eval reports today reflects that one real, small-scale run against
  `mem0_direct`, not a reproduction of jamebobob's own 97.8%/808 production-scale figures.
- **Extending this eval:** adding more cases means adding entries to the fixture file with the
...
**What a real run showed.** `mem0_direct` (self-hosted `mem0ai`, in-process, backed by local
Qdrant + OpenAI) has been run against this eval's 7 fixture cases: 7/7 classified
`SILENT_OVERWRITE` -- every case's retrieved content contained only `updated_value`, never
`initial_value`, meaning mem0 resolved every contradiction with no trace of the prior value
surviving anywhere in the response. 0% of cases were `FLAGGED`.
...
comparison, and it requires an adapter (like `MemPalaceAdapter`) that declares more than one
`supported_modes` entry. **A run has now been performed against a live backend for the
single-mode case**: `mem0_direct` (self-hosted `mem0ai`, in-process, backed by local Qdrant +
OpenAI, which declares no `supported_modes` and so runs only the synthetic `"default"` mode) --
5 cases, mean fidelity 31.6%. `fidelity_drop_pp` itself remains unmeasured, since that number
needs at least two modes to compare and no multi-mode adapter (MemPalace included) has been run
live yet.
...
| `mem0_direct_adapter.py` (`Mem0DirectAdapter`, direct in-process library, embedder/vector-store selection) | **High on what the installed `mem0ai==2.0.12` package's code actually does (confirmed by reading its real, installed source, not documentation or the GitHub issues), Low on live end-to-end behavior** | Every embedder-dims-forwarding and vector-store vector=None-guard claim below was confirmed by reading the *installed* `mem0.embeddings.{aws_bedrock,fastembed,gemini,openai}.py` and `mem0.vector_stores.{redis,valkey,qdrant}.py` source directly, and `tests/test_mem0_direct_adapter.py` exercises those real classes with only the vendor SDK/wire-client boundary mocked (`boto3`, `openai`, `google.genai`, `fastembed.TextEmbedding`, `redis`/`valkey`/`qdrant_client`) -- see "Mem0DirectAdapter and the retired Kuzu bug" and "Qdrant support: #4297 and #4453" below for the full findings, including two surprising results: `graph_store`/Kuzu support does not exist in this package version at all, and #4297's dimension-mismatch bug class is confirmed **still reachable** (not fixed) in the installed Qdrant path, even though its companion bug #4453 (threshold inversion) is confirmed fixed comprehensively across every vector store. | `tests/test_mem0_direct_adapter.py`'s own unit tests still mock the vendor boundary -- `boto3`/`openai`/`google.genai`/`fastembed.TextEmbedding`/`redis`/`valkey`/`qdrant_client` are never reached for real inside `pytest`, same convention this document already states for `mem0_adapter.py`. Separately, outside `pytest`, a real live benchmark run has now been performed against `mem0_direct` backed by a genuine local Qdrant instance and the real OpenAI API (the contradiction, compression, and extraction_quality evals -- see those sections above and below for the results). That run also surfaced a real bug in `mem0ai` itself, unrelated to the #4297/#4453 findings below: the package's own default LLM (`gpt-5-mini`, hardcoded in `mem0/llms/openai.py`) is not recognized by its own reasoning-model detection set in `mem0/llms/base.py` (which checks for the different string `gpt-5o-mini`), so mem0 sends a `temperature` the model rejects and every extraction call 400s out of the box for anyone with just `OPENAI_API_KEY` set -- `Mem0DirectAdapter` now works around it via `is_reasoning_model=True`; see `mem0_direct_adapter.py`'s "Default LLM extraction is broken out of the box" docstring section for the full writeup. `_read_raw_embedding_bytes()`'s raw-client inspection (used by `update_metadata_only()` to derive `CorruptionSignal.CLEAN`/`VECTOR_ZEROED`) reaches into each vector store's private-ish `schema`/`prefix`/`client` attributes rather than a documented public API, since `VectorStoreBase.get()` does not expose raw vector bytes at all -- confirmed by reading the base class, but still an internals-reaching workaround that could break on a future `mem0ai` release's internal refactor (it would fail closed, into `NOT_APPLICABLE`, not silently misreport). The #4297 finding was confirmed by mocking `QdrantClient` and inspecting the `VectorParams(size=...)` passed to `create_collection` -- never against a real running Qdrant server, so nothing here demonstrates the actual "Bad Request on insert" HTTP response a live server would return. |
...
a branch inside `ZepGraphitiAdapter`. It exists specifically because seven real, independently-verified
...
`attributes`/`reference_time`, now fixed upstream), #1001 (FalkorDB's old `add_triplet()`
silently no-oping and never setting edge endpoint UUIDs, now closed via #1013), #1222 (closed,
superseded by #1475 -- FalkorDB's `build_fulltext_query()` appending empty parentheses to the
group filter for an empty sanitized query, producing invalid RediSearch syntax), and #1183
(merged -- the same `build_fulltext_query()`'s `sanitize()` helper omitting pipe/slash/backslash
from its character map, letting those characters survive into a malformed, empty-token RediSearch
OR query) -- all live entirely inside the self-hosted `graphiti-core` library layer that
`ZepGraphitiAdapter`'s hosted REST calls can never reach. See that new adapter's module docstring
for the full citation trail (#1302/#836/#1013/#1001 were confirmed by fetching and reading the real
source from `getzep/graphiti`'s `main` branch on GitHub during this build; #920 was confirmed via
the issue's own filed traceback; #1222/#1183 were confirmed via each issue/PR's own filed
reproduction and diff -- none by running any of them against a live database) and, plainly, what
this build could NOT verify: no Neo4j or FalkorDB instance was ever started or reached in this
environment, so nothing here demonstrates any of the seven bugs actually reproducing end-to-end.
`ConflictSignal.EDGE_INTEGRITY_VIOLATION`, `CrashSignal` (`UNPACK_ERROR`/`TYPE_COMPARISON_ERROR`/
`QUERY_SANITIZATION_ERROR`/`UNKNOWN`), and `MemoryRecord.attributes` (all `adapters/base.py`) are
the shared-interface additions this adapter needed: the first lets `evals/contradiction.py`'s
`classify_case()` flag a structurally broken edge (missing `source_node_uuid`/`target_node_uuid`)
as its own distinct signal instead of folding it into an ordinary text-classification miss;
`CrashSignal` (added to close the #836/#920 "identical opaque error" gap, later extended for
#1222/#1183 -- see below) lets `store()`'s and `query()`'s `BackendAPIError` carry which known
crash *shape* the underlying exception matched, instead of every internal graphiti-core failure
surfacing as the same undifferentiated string; the last lets a backend's structured per-record
properties (graphiti-core's `EntityEdge.attributes`) survive the adapter boundary at all. All three
are purely additive to the shared interface -- no existing adapter's behavior changes.
...
**Crash classification for #1222 and #1183, and wiring `query()` into `_classify_crash()`
(`CrashSignal.QUERY_SANITIZATION_ERROR`, added 2026-07-16).** Before this change, `_classify_crash()`
existed but `query()` never called it at all -- unlike `store()`, `query()`'s `except Exception`
handler wrapped every failure into `BackendAPIError(self.name, str(exc))` with `crash_signal`
left at its default `None`, the same opaque-error gap `store()` had before the #836/#920 work
above closed it there. This was the actual, real gap two independently-documented upstream bugs
fall into: getzep/graphiti#1222 (closed, superseded by #1475) and getzep/graphiti#1183 (merged)
both live in FalkorDB's RediSearch fulltext-query path
(`graphiti_core.driver.falkordb_driver.FalkorDriver`'s `sanitize()`/`build_fulltext_query()`) --
reachable only through `query()`'s `Graphiti.search()` call, never through `store()`'s
`add_episode()`. `_classify_crash()` now recognizes a third shape, `CrashSignal
.QUERY_SANITIZATION_ERROR`: message contains both `"redisearch"` and `"syntax error"` (matching the
real, filed message text `"RediSearch: Syntax error at offset N near ..."` both issues report), with
no `isinstance` check -- the real exception FalkorDB raises here is a `redis`-client
`ResponseError`, and this adapter deliberately does not import the optional `redis`/`falkordb`
packages just to type-check against them. `query()`'s `except Exception` handler now calls
`_classify_crash(exc)` unconditionally (rather than store()'s `(ValueError, TypeError)` pre-filter,
since this bug class's real exception type is neither) and attaches the result to the raised
`BackendAPIError.crash_signal`. This is unit-tested in `tests/test_adapters.py` against fake
`_GraphitiProtocol` clients whose `search()` raises the exact message text each issue's own filed
reproduction reports -- one for #1222's empty-query trigger, one for #1183's pipe/slash-character
trigger -- plus guard tests confirming a message containing only "syntax error" (no "redisearch")
and a generic `RuntimeError` both still classify as `UNKNOWN`, proving the classifier distinguishes
this new shape from both the unrelated-failure case and from #836/#920's shapes. **What this
honestly closes, and what it does not:** the same caveat as the #836/#920 paragraph above, applied
to `query()` -- it proves this adapter recognizes this crash shape once graphiti-core has already
raised it, it does not prevent the crash, does not patch graphiti-core, and does not prove this
adapter's own `query()` calls actually trigger a RediSearch syntax error against a live FalkorDB
instance; that still requires a live deployment this build never had.
...
**What this build did not, and could not, verify for either bug.** `tests/test_mem0_direct_adapter.py`'s
own Qdrant-related unit tests still mock `QdrantClient` at the wire boundary and never reach a real
server. A real local Qdrant server has since been started and reached, separately, for a live
benchmark run against `mem0_direct` (see the contradiction, compression, and extraction_quality
sections above), but that run exercised `Mem0DirectAdapter`'s normal `store()`/`query()` path, not
a deliberately mis-sized collection -- `_build_vector_store_config()` always threads the resolved
embedder dimension through correctly (see above), so nothing in that live run demonstrates the
actual `Bad Request` HTTP response a live server returns on a genuine dimension mismatch, or
confirms end-to-end that a real insert against a live collection fails the way #4297 describes when
the config *is* mismatched. The `mem0ai==2.0.12` PyPI release is the
...
instance was ever started or reached. Of the seven bugs motivating this adapter's addition, two
...
ever run against an older, affected version. Of the remaining five, `update_communities=True` is
...
a contributor with a live instance could use to observe the effect, not reproduce it here. #1222
and #1183 are the same story as #836/#920: both live inside FalkorDB's real `sanitize()`/
`build_fulltext_query()` fulltext-query path, which this adapter's mocked `_GraphitiProtocol`
double's `search()` never actually executes -- nothing in this build demonstrates this adapter's
own `query()` calls trigger a RediSearch syntax error against a live FalkorDB instance either. If a
reader wants confidence that this adapter reproduces any of these seven issues, the honest answer
...
"reproducing" any of the seven issues above: `store()`'s and `query()`'s `CrashSignal`
classification (`CrashSignal.UNPACK_ERROR` for #836's exact `ValueError` message shape,
`CrashSignal.TYPE_COMPARISON_ERROR` for #920's exact `TypeError` message shape, `CrashSignal
.QUERY_SANITIZATION_ERROR` for #1222's/#1183's shared `RediSearch: Syntax error` message shape) is
real, tested logic that runs on whatever exception a live instance *would* raise, once it raises
it -- proven by fake `_GraphitiProtocol` clients in `tests/test_adapters.py` that raise the exact
message strings each issue's own filed GitHub traceback/reproduction reports, plus negative tests
confirming an unrelated `ValueError`/`TypeError`, a generic `RuntimeError`, and a message containing
only "syntax error" without "redisearch" all correctly fall through to `CrashSignal.UNKNOWN` rather
than being misclassified. That is a real, narrow, honestly-scoped capability: distinguishing these
three specific known bug classes from an opaque generic failure, and from each other. It is not,
and should not be read as, evidence that this adapter reproduces any of the three crashes against
