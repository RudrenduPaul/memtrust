...
None of the distribution mechanics above matter if the tool being distributed can't be trusted. An agent calling `memtrust run` autonomously, parsing the JSON, and deciding what to do next has no human in the loop to notice if a result is quietly wrong. That's the actual stakes of Part 1's discovery: a fictional API call inside that adapter meant every downstream consumer of its output, human or agent, had been acting on a claim nobody had checked.
